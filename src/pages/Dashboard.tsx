...
// Mock Data cho biểu đồ lớn
const mockRealtimeData = [
  { time: '22:11:31', temp: 30, humid: 80, light: 40 },
  { time: '22:11:33', temp: 32, humid: 78, light: 45 },
  { time: '22:11:35', temp: 31, humid: 75, light: 42 },
  { time: '22:11:37', temp: 33, humid: 72, light: 40 },
  { time: '22:11:39', temp: 32, humid: 75, light: 38 },
  { time: '22:11:41', temp: 34, humid: 78, light: 41 },
  { time: '22:11:43', temp: 35, humid: 82, light: 43 },
  { time: '22:11:45', temp: 32, humid: 80, light: 40 },
];
...
export default function Dashboard() {
  const [leds, setLeds] = useState<{ [key: string]: boolean }>({ led1: true, led2: true, led3: false });

  const toggleLed = (led: string) => setLeds(prev => ({ ...prev, [led]: !prev[led] }));
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <StatCard title="Ánh sáng" value="400" unit="Lux" icon={Lightbulb} colorClass="#F97316" iconBgClass="bg-orange-100" iconColorClass="text-orange-500" gradientId="colorLight" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Main Chart */}
        <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          <h3 className="text-sm font-semibold text-gray-500 mb-6 uppercase tracking-wider">Biểu đồ theo thời gian thực</h3>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={mockRealtimeData} margin={{ top: 5, right: 30, left: -20, bottom: 5 }}>
                ...
                <Line type="monotone" dataKey="temp" name="Nhiệt độ" stroke="#EF4444" strokeWidth={3} dot={{ r: 4, strokeWidth: 2 }} activeDot={{ r: 6 }} />
                <Line type="monotone" dataKey="humid" name="Độ ẩm" stroke="#3B82F6" strokeWidth={3} dot={{ r: 4, strokeWidth: 2 }} activeDot={{ r: 6 }} />
                <Line type="monotone" dataKey="light" name="Ánh sáng" stroke="#F59E0B" strokeWidth={3} dot={{ r: 4, strokeWidth: 2 }} activeDot={{ r: 6 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
              ...
              return (
                <div key={num} className="flex items-center justify-between">
                  ...
                  <button 
                    ...
                    className={`relative inline-flex h-8 w-14 items-center rounded-full transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#2a1b4d] ${isOn ? 'bg-[#2a1b4d]' : 'bg-gray-200'}`}
                  >
